defmodule Bout.AccountsDeleteTest do
  use Bout.DataCase, async: true

  alias Bout.{Accounts, Contests, Repo}
  ...
  test "an accepted duel is cancelled with everyone refunded" do
    ...
    befriend(a, b)
    {:ok, _} = Bout.Coins.grant_signup(a.id)
    {:ok, _} = Bout.Coins.grant_signup(b.id)
    a_before = Bout.Coins.balance(a.id)
    b_before = Bout.Coins.balance(b.id)
    ...
    assert Repo.get(Duel, duel.id).status == "cancelled"
    assert Bout.Coins.balance(a.id) == a_before
    assert Bout.Coins.balance(b.id) == b_before
  end
