defmodule Bout.DevToolsTest do
  use Bout.DataCase, async: false

  alias Bout.{Accounts, DevTools, Drafts, Repo}
  alias Bout.Contests.Duel
  alias Bout.Drafts.Pick
  alias Bout.Sports.Player

  setup do
    prev = Application.get_env(:bout, :dev_routes)
    Application.put_env(:bout, :dev_routes, true)
    on_exit(fn -> Application.put_env(:bout, :dev_routes, prev) end)
    :ok
  end
  ...
  test "is blocked outside dev (dev_routes off)" do
    Application.put_env(:bout, :dev_routes, false)
    {duel, _c, _o} = drafted_with_rosters()
