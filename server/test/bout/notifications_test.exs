defmodule Bout.NotificationsTest do
  # async: false — swaps the Notifications app env per test.
  use Bout.DataCase, async: false

  alias Bout.{Accounts, Notifications}

  @stub Bout.NotificationsTest.Stub

  setup do
    prev = Application.get_env(:bout, Bout.Notifications)

    Application.put_env(:bout, Bout.Notifications,
      enabled: true,
      push_url: "https://expo.test/push",
      req_options: [plug: {Req.Test, @stub}, retry: false]
    )

    on_exit(fn -> Application.put_env(:bout, Bout.Notifications, prev) end)
    :ok
  end
