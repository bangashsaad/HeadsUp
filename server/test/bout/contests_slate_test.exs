defmodule Bout.ContestsSlateTest do
  # async: false — these tests override the app-env slate client.
  use Bout.DataCase, async: false

  alias Bout.{Accounts, Contests, Drafts, Repo}
  alias Bout.Contests.Duel
  alias Bout.Social.Friendship
  alias Bout.Sports.{Player, Slate}
  ...
  setup do
    Application.put_env(:bout, :slate_client, StubSlateClient)
    on_exit(fn -> Application.delete_env(:bout, :slate_client) end)
  ...
  describe "the slate freezes the scoring window" do
    ...
    test "a slate-less duel keeps the legacy anchored-at-completion window", %{a: a, b: b} do
      ...
      assert seconds == Application.get_env(:bout, :scoring_window_seconds, 86_400)
    end
  end
  ...
  describe "expire_stale/1 (the Janitor's sweep)" do
    ...
    test "an expired pending challenge sends the stake home", %{a: a, b: b} do
      {:ok, _} = Bout.Coins.grant_signup(a.id)
      before = Bout.Coins.balance(a.id)

      {:ok, duel} = create(a, b, %{"stake_coins" => 100})
      assert Bout.Coins.balance(a.id) == before - 100

      age(Repo.get(Duel, duel.id), 30)
      assert %{pending: 1} = Contests.expire_stale(24)
      assert Bout.Coins.balance(a.id) == before
    end
  end
end
