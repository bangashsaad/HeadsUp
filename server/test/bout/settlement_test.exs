defmodule Bout.SettlementTest.StubStats do
  @moduledoc false
  @behaviour Bout.Settlement.StatsProvider
  alias Bout.Settlement.Window
...
defmodule Bout.SettlementTest do
  # async: false -> shared sandbox so the app-supervised worker can hit the DB.
  use Bout.DataCase, async: false

  alias Bout.{Accounts, Drafts, Repo, Settlement}
  alias Bout.Contests.Duel
  alias Bout.Drafts.Pick
  alias Bout.Settlement.{Result, Worker}
  alias Bout.Sports.Player

  setup do
    Application.put_env(:bout, :stats_provider, Bout.SettlementTest.StubStats)
    on_exit(fn -> Application.put_env(:bout, :stats_provider, Bout.Settlement.Stats.Mock) end)
    :ok
  end
  ...
  defp group_drafted_duel(players, rules, window_end) do
    ...
    for {u, seat} <- Enum.with_index(players) do
      Repo.insert!(%Bout.Contests.Participant{duel_id: duel.id, user_id: u.id, seat: seat, status: "accepted"})
    end
