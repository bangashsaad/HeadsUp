defmodule Bout.HomeTest do
  use Bout.DataCase, async: true

  alias Bout.{Accounts, Home, Repo}
  ...
  defp group_pending(host, invitees) do
    ...
    Repo.insert!(%Bout.Contests.Participant{duel_id: d.id, user_id: host.id, seat: 0, status: "accepted"})

    for {{u, status}, i} <- Enum.with_index(invitees, 1) do
      Repo.insert!(%Bout.Contests.Participant{duel_id: d.id, user_id: u.id, seat: i, status: status})
    end
