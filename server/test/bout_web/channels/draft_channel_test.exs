defmodule BoutWeb.DraftChannelTest do
  use BoutWeb.ChannelCase, async: false

  alias Bout.{Accounts, Drafts, Repo}
  alias Bout.Contests.Duel
  alias BoutWeb.UserSocket
  ...
  defp pool do
    for {pos, pi} <- Enum.with_index(~w(PG SG SF PF C)), n <- 1..3 do
      Repo.insert!(%Bout.Sports.Player{
        sport: "wnba",
  ...
  defp first_player_id do
    import Ecto.Query
    Repo.one(from p in Bout.Sports.Player, where: p.sport == "wnba", limit: 1, select: p.id)
  end
end
