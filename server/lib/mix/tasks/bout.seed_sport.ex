defmodule Mix.Tasks.Bout.SeedSport do
  ...
  FPPG pass (resilient per player).
      ...
      mix bout.seed_sport wnba
      mix bout.seed_sport mlb

  `bout.seed_wnba` / `bout.seed_mlb` are thin aliases for this.
  """
  use Mix.Task
  ...
  def run(_), do: Mix.raise("usage: mix bout.seed_sport <#{Enum.join(Client.leagues(), "|")}>")
