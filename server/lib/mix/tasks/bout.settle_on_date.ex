defmodule Mix.Tasks.Bout.SettleOnDate do
  ...
  can test the live ESPN settlement pipeline against a past final game.
      ...
      mix bout.settle_on_date <duel_id> <YYYY-MM-DD>

  Dev-only (guarded by `:dev_routes`). See `Bout.DevTools.settle_on_date/3`.
  """
  use Mix.Task
  ...
  def run([duel_id, date]) do
    case Bout.DevTools.settle_on_date(String.to_integer(duel_id), date) do
      {:ok, result, duel} ->
        Mix.shell().info("""
  ...
  def run(_), do: Mix.raise("usage: mix bout.settle_on_date <duel_id> <YYYY-MM-DD>")
end
