defmodule Bout.Contests do
  ...
  alias Bout.Repo
  alias Bout.Accounts.User
  alias Bout.Coins
  alias Bout.Social
  alias Bout.Contests.{Duel, Participant, Scoring}
  alias Bout.Drafts.Lineup
  alias Bout.Sports.{Player, Season, Slate}
  ...
  defp scoring_window(_duel, now) do
    window_seconds = Application.get_env(:bout, :scoring_window_seconds, 86_400)
  ...
  def expire_stale(cutoff_hours \\ 24) do
    ...
    lobby_ids =
      from(d in Duel,
        left_join: dr in Bout.Drafts.Draft,
        on: dr.duel_id == d.id,
        left_join: pk in Bout.Drafts.Pick,
  ...
  defp expire_pending(duel_id) do
    ...
    |> case do
      {:ok, %{duel: fresh}} ->
        Bout.Notifications.notify_user(
          fresh.challenger_id,
          "Challenge expired 🕰️",
