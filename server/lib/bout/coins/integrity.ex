defmodule Bout.Coins.Integrity do
  @moduledoc """
  ...
  the Bout-specific invariant — that the escrow account holds exactly the
  ...
  defp escrow_mismatch do
    escrow =
      ...
      |> then(fn account ->
        from(b in Bout.Coins.Balance, where: b.account_id == ^account.id, select: -b.amount)
        |> Repo.one() || 0
      end)
