defmodule Bout.Settlement do
  ...
  alias Bout.Drafts
  alias Bout.Drafts.Draft
  alias Bout.Settlement.{Engine, Result, Window}
  ...
  defp notify_settled(%Duel{} = duel, standings) do
    ...
    for s <- standings do
      ...
      Bout.Notifications.notify_user(s.user_id, title, body, %{type: "result", duel_id: duel.id})
    end
  ...
  defp provider(sport) do
    Application.get_env(:bout, :stats_providers, %{})
    |> Map.get(sport) || Application.get_env(:bout, :stats_provider, Bout.Settlement.Stats.Mock)
  end
end
