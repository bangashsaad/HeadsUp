defmodule Bout.Accounts do
  ...
  alias Bout.Repo
  alias Bout.Accounts.{User, UserToken}
  ...
  defp send_code_email(to, subject, body) do
    sender = Application.get_env(:bout, :mail_from, {"Bout", "onboarding@resend.dev"})
    ...
    case Bout.Mailer.deliver(email) do
      {:ok, _} ->
        :ok
  ...
  defp do_delete_account(%User{} = user) do
    ...
    Bout.Contests.evacuate_user(user)
    Bout.Social.delete_all_friendships(user)
    Repo.delete_all(UserToken.by_user_and_contexts_query(user, :all))
