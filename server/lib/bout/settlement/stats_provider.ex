defmodule Bout.Settlement.StatsProvider do
  @moduledoc """
  ...
  `Bout.Contests.Scoring` chart keys (so the scoring engine can dot-product
  them against the frozen chart with no translation).

  Stage 5a ships `Bout.Settlement.Stats.Mock`; 5b drops in a real WNBA feed by
  config swap only (`config :bout, :stats_provider, ...`).
  """
