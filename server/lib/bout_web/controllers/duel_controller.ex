defmodule BoutWeb.DuelController do
  use BoutWeb, :controller

  alias Bout.{Contests, Settlement}
  alias Bout.Contests.Duel
  alias Bout.Settlement.Result

  import BoutWeb.UserAuth, only: [require_verified_email: 2]

  plug :put_view, json: BoutWeb.DuelJSON
  ...
  def result(conn, %{"id" => id}) do
    ...
    with %Duel{} = duel <- Contests.get_duel(user, id),
      ...
      conn
      |> put_view(json: BoutWeb.ResultJSON)
    ...
    case Contests.get_duel(user, id) do
      ...
      %Duel{} ->
        case Settlement.live_result(id) do
          {:ok, live} ->
            conn |> put_view(json: BoutWeb.LiveJSON) |> render(:show, live: live, current_user_id: user.id)
