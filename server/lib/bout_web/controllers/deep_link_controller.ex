defmodule BoutWeb.DeepLinkController do
  @moduledoc """
  ...
  @team_id "VFX855N66M"
  # The Bout identity ships with the next native build. The LEGACY id still
  # matches the .ipa this page serves today, so the manifest keeps using it
  # while AASA claims BOTH — universal links work before and after the switch.
  @bundle_id "com.boutfantasy.app"
  @legacy_bundle_id "com.headsupfantasy.app"
  ...
  def aasa(conn, _params) do
    ...
    legacy_id = "#{@team_id}.#{@legacy_bundle_id}"
  ...
  def install(conn, _params) do
    ...
    <head>
      ...
      <title>Install Bout Fantasy</title>
      <style>
    ...
    <dict>
      ...
      <array>
        <dict>
          ...
          <key>metadata</key>
          <dict>
            <key>bundle-identifier</key><string>#{@legacy_bundle_id}</string>
            <key>bundle-version</key><string>#{@app_version}</string>
            <key>kind</key><string>software</string>
            <key>title</key><string>Bout Fantasy</string>
          </dict>
        </dict>
      </array>
    </dict>
  ...
  def fallback(conn, _params) do
    ...
    <body>
      <div class="card">
        <div class="mark">HU</div>
        <h1>You've been challenged on Bout</h1>
