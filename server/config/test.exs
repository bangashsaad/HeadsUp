...
config :bout, :settlement_interval_ms, 3_600_000

# Same deal for the stale-duel janitor: tests call Contests.expire_stale/1.
config :bout, :janitor_interval_ms, 3_600_000
...
config :bout, Bout.Notifications, enabled: false, req_options: []
...
config :bout, Bout.Repo,
  ...
  password: "postgres",
  hostname: "localhost",
  database: "bout_test#{System.get_env("MIX_TEST_PARTITION")}",
...
config :bout, Bout.Mailer, adapter: Swoosh.Adapters.Test
config :bout, :require_verified_email, false
